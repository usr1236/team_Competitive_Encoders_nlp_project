...
TEST_JSON = "test_data/subtask 1/test_data_subtask1.json"
EVAL_SCRIPT = "evaluation_kit/task 1 & 3/evaluation_script.py"
OUT_DIR = "results_cot_llama"
...
Think step by step about the logical structure, then provide your final label.
Output format:
...
def parse_validity(generated_text: str) -> bool:
    ...
    final_match = re.search(r'final\s*answer\s*:\s*(valid|invalid)\b', text)
    if final_match:
        return final_match.group(1) == "valid"
...
if __name__ == "__main__":
    ...
    print(f"Loaded {len(items)} items from {TEST_JSON}")
    ...
    run_official_eval(EVAL_SCRIPT, TEST_JSON, PRED_PATH, EVAL_OUT_PATH)
